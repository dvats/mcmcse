...
multiESS_no_sym_log <- function(x, covmat = NULL, g = NULL, ...) {
  chain <- as.matrix(x)
	...
	if(!is.matrix(covmat))
	{
	  ...
	  covmat <- mcse.multi(chain, size = bs, r=1,  ...)$cov
	} 
...
benchmark(replications = 10, multiESS(chain), multiESS_new(chain), multiESS_eigen_log(chain), multiESS_eigen_symmetric(chain), multiESS_no_sym_log(chain))
```
```{r}
...
print(multiESS(chain))
print(multiESS_eigen_log(chain))
# chain = make_VAR(20, 1e4)
# benchmark(replications = 10, multiESS(chain), multiESS_new(chain), multiESS_eigen_log(chain), multiESS_eigen_symmetric(chain), multiESS_no_sym_log(chain))
# 
