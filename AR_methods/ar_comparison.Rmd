...
batchSize_threshold <- function(x, method = "bm", g = NULL)
{
...
log_unnormalised_posterior_new <- function(X)  {
  #ans = (-0.5) * sum(X^2)
...
RWMH_new <- function(n, init, h)  {
    ...
    for(t in 2:n) {
      ...
      log_ratio = log_unnormalised_posterior_new(prop) - log_unnormalised_posterior_new(output[t-1,]) # work with log for numerical stability
...
init = numeric(p)
h = 0.5
ans = RWMH_new(n, init, h)
```

```{r}
benchmark(replications = 1e1, batchSize(ans), batchSize_threshold(ans))
```
