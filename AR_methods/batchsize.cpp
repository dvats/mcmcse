...
List arp_approx(vec x, vec xacf, int max_order, uword n) {
...
// [[Rcpp::export]]
double batchsize_cpp(mat chain, mat xacf_mat, int max_order, String method) {
  ...
  for(int i = 0; i<p; i++)  {
    ...
    arp_output = arp_approx(chain.col(i), xacf_mat.col(i), max_order, n);
