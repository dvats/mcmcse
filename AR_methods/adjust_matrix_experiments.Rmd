...
(d) mcse.multi(r = 1) but replace the diagonals of this with diag( mcse.multi(r = 3, adjust = FALSE) )
      — that is calculate regular BM, but replace the diagonals of BM with the diagonals of lugsail.
(e) mcse.multi(r=1, adjust = FALSE)
...
for(i in 1:reps)  {
  ...
  ESS_e[i] = multiESS_eigen_log(chain, r=1, adjust = FALSE, size = bs)
}

print(c(sum(is.na(ESS_a)), sum(is.na(ESS_b)), sum(is.na(ESS_c)), sum(is.na(ESS_d)), sum(is.na(ESS_e))))

boxplot(ESS_a, ESS_b, ESS_c, ESS_d, ESS_e, names = c("ESS_a", "ESS_b", "ESS_c", "ESS_d", "ESS_e"))
abline(h = ESS_truth)
```
